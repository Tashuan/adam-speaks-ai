...
- OAuth client credentials for the agent to discover templates and start hosted registration for a user who has not created an Adam account yet.
- OAuth authorization-code with PKCE/delegated access when an agent acts for an existing Adam user.

Dynamic client secrets belong in the trusted agent environment only. They must not be written into the user's app, browser bundle, `.env` file, logs, or repository. The hosted Google registration is what attaches the resulting avatar to the user's Adam account.
